...
export default async function CountryPage({
  ...
  for (const dish of results) {
    categoryCounts.set(
      dish.category,
      (categoryCounts.get(dish.category) ?? 0) + 1
    )
    ...
    <div>
      ...
      {ingredientCombos.length > 0 && (
        ...
        >
          ...
          {ingredientCombos.map((combo) => (
            <Link
              key={combo.tagId}
              href={comboIngredientPath(combo.countryId, combo.tagId)}
              style={{
                color: "#7a4f2a",
                fontSize: "0.8125rem",
                background: "#f0e6d6",
